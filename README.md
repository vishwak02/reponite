...
Reponite exposes **17 MCP tools** so an agent can read, understand, and safely change code — including ROS runtime wiring — without opening source files.
...
**Tools:** `investigate` · `search` · `semsearch` · `grep` · `usages` · `topics` · `context` · `brief` · `compat` · `diff` · `rootcause` · `rootcause_trace` · `ximpact` · `blast_radius` · `verify_edit` · `repos` · `refs`
