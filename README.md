...
Reponite exposes **16 MCP tools** so an agent can read, understand, and safely change code without opening source files.
...
**Tools:** `investigate` · `search` · `semsearch` · `grep` · `usages` · `context` · `brief` · `compat` · `diff` · `rootcause` · `rootcause_trace` · `ximpact` · `blast_radius` · `verify_edit` · `repos` · `refs`
