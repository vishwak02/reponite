...
> (§8A.6) ships as a git-blame provider. **Per-caller signature-skew (§8B.3) is built** (2026-07-19):
...
**As built (2026-07-19, Phase 6b).** When a repo carries a SCIP index (`index.scip` at its root), reponite reads it at index time (`internal/scip`, PURE — a SCIP index is a protobuf file, and the six fields needed decode with a small stdlib wire reader, so no dependency and no build tag). Each locally defined symbol records its **moniker** — SCIP's globally unique symbol string, e.g. ``scip-go gomod github.com/acme/api v1.2.0 `pkg/user`/GetUser().`` — and each reference to a moniker defined *elsewhere* becomes an external ref carrying that moniker (`scip-resolved`@0.95). `XImpact` then runs a **tier 0** ahead of the module and name tiers: match callers whose `target_symbol` equals a definition's moniker. That needs no module comparison at all — a moniker either is this symbol or isn't — so a same-named symbol in another project can never collide. The tier is reported (`N caller(s) SCIP-resolved by symbol moniker`), never implied. **Fail-safe by construction:** monikers are matched by exact string equality between two independently produced indexes, so a decode bug or a producer-version mismatch can only *lose* matches (degrading to the existing tiers), never invent a high-confidence edge. A missing index is the normal case; a corrupt one is reported on stderr and skipped, and indexing still succeeds.
...
Both halves shipped. **Capture:** a caller file's import bindings (`imports.go`, per language) resolve its qualified/from-imported call sites (`Symbol.QualifiedCalls`) to `(module_path, name)` external references at index time (`resolveExternalRefs`, `import-resolved`@0.75), stored per-ref in `external_refs` and cleared on reindex. A repo's `module_path` is detected from its ecosystem manifest (`module.go`: `go.mod`/`package.json`/`pyproject.toml`/`pom.xml`, root-most wins). **Query:** `XImpact` fuses tier 1 module-resolved callers (match `external_refs` on the target's own `module_path` — so a same-named symbol in an unrelated module does **not** collide) with tier 2 the original name-based `unresolved-external` scan (fallback, deduped by caller); each caller carries its `resolution_method` so the precise and heuristic tiers are distinguishable. The `serve` **MultiStore** fans `ExternalRefsTo` across several per-repo stores for a live multi-dir fleet view. **Per-caller skew (as built, 2026-07-19):** at index time each external ref also records `target_signature_hash` — the target's signature as seen from the store being written (resolvable in a shared/fleet store or monorepo; "" in an isolated per-repo store — unknown, never guessed; ambiguous names likewise capture nothing). At query time `XImpact` compares each captured contract against the target's *current* signature (per definition identity, at HEAD or its latest ref) and labels every module-resolved caller `expected_signature: current | stale`, with a `stale_callers` rollup — "3 of 4 callers still expect the old shape". **Still deferred:** a persistent cross-run `global.db` (so separately-invoked CLI indexes share a registry without `serve` — also what makes skew capture fleet-wide); SCIP-resolved cross-boundary edges are BUILT (§8B.4 "As built"); what remains there is consuming SCIP's *own* relationship graph (implementations/overrides) rather than only definitions and references.
...
### 8D.3 Output
`reponite_topics(topic?, repo?, ref?)` — no `topic`: the whole comms map, connected edges first; with `topic`: that one name's producers and consumers. Each endpoint carries `repo`, `path`, `line`, `role`, the normalized `name` + `raw` (as-written), `msg_type` when captured — labeled by `msg_type_source`: **template** (C++ `<T>`), **callback-param** (ROS1 `subscribe()` carries no template, the middleware infers the type from the callback, so it is recovered from the callback's message parameter — `const T::ConstPtr&`, `shared_ptr<T const>`, `MessageEvent<T>` — found in the same file; best-effort, never guessed cross-file), or **positional-arg** (rospy/rclpy pass the type class positionally) — and the enclosing symbol `in` (a hop back into the call graph). Type matching is cross-language (`sensor_msgs::LaserScan` ≡ `sensor_msgs.msg.LaserScan` ≡ `LaserScan` by trailing segment). Each group carries a `connected` flag and a `confidence`.
...
CREATE TABLE external_refs (
  ...
  target_signature_hash TEXT,        -- captured target contract → per-caller skew (§8B.3)
  target_symbol TEXT,                -- SCIP moniker → symbol-resolved match (§8B.4)
  resolution_method TEXT NOT NULL,   -- scip-resolved | import-resolved | unresolved-external
